...
local MINOR_VERSION = 90000 + (tonumber(("$Revision: 107 $"):match("%d+")) or 0)
...
local HEROIC = DUNGEON_DIFFICULTY2 or PLAYER_DIFFICULTY2
local SCENARIO = GUILD_CHALLENGE_TYPE4 or "Scenario"
local ACCOUNT = "Account" -- can't find a localization for this...
local shorttags = {
	[ELITE] = '+',
	[GROUP] = 'g',
	[PVP] = 'p',
...
do
	local hooks_active = true
	function lib:UnhookDialogs()
		hooks_active = false
	end
	hooksecurefunc("AcceptQuest", function()
		if not hooks_active then return false end
		lib.npc = UnitName("npc")
		lib.npc_is_player = UnitIsPlayer("npc")
	end)
	hooksecurefunc(StaticPopupDialogs["ABANDON_QUEST"], "OnAccept", function()
		local name = GetAbandonQuestName()
		local uid = lib:GetQuest(name)
		Debug("Quest_Abandoned", name, uid)
		lib.callbacks:Fire("Quest_Abandoned", name, uid)
	end)
	hooksecurefunc(StaticPopupDialogs["ABANDON_QUEST_WITH_ITEMS"], "OnAccept", function()
		local name = GetAbandonQuestName()
		local uid = lib:GetQuest(name)
		lib.callbacks:Fire("Quest_Abandoned", name, uid)
	end)
...
local objects_pattern = '^' .. QUEST_OBJECTS_FOUND:gsub('(%%%d?$?.)', '(.-)') .. '$' --QUEST_OBJECTS_FOUND = "%d/%d %s"
local monsters_pattern = '^' .. QUEST_MONSTERS_KILLED:gsub('(%%%d?$?.)', '(.-)') .. '$' --QUEST_MONSTERS_KILLED = "%d/%d %s slain"
local faction_pattern = '^' .. QUEST_FACTION_NEEDED:gsub('(%%%d?$?.)', '(.-)') .. '$' --QUEST_FACTION_NEEDED = "%s / %s %s"
local player_pattern = '^' .. QUEST_PLAYERS_KILLED:gsub('(%%%d?$?.)', '(.-)') .. '$' --QUEST_PLAYERS_KILLED = "%d/%d %s Players slain"
...
function frame:QUEST_LOG_UPDATE()
	local now = GetTime()
	if not frame.lastUpdate or (now - frame.lastUpdate) > TOOLTIP_UPDATE_TIME then
	...
	if numEntries > 0 then
		...
		while quest_count < numQuests do
			...
			if header then
				...
				local tagid, tag = GetQuestTagInfo(unique_id)
				...
				q.tag = tag
				q.group = group
				q.daily = frequency == LE_QUEST_FREQUENCY_DAILY
				q.weekly = frequency == LE_QUEST_FREQUENCY_WEEKLY
				if frequency and not tag then
				...
				if numObjectives and numObjectives > 0 then
					...
					for o = 1, numObjectives do
						...
						elseif qtype == 'reputation' then
							numItems, numNeeded, desc = desc:match(faction_pattern)
...
local function worth_sending() return lib.worth_sending and (not IsInRaid()) and (GetNumGroupMembers() > 0) end
lib:RegisterCallback("Quest_Gained", function(event, title, uid, objectives)
	if not worth_sending() then return end
...
function lib:GetQuestObjective(uid, desc)
	...
	if q and q[desc] then
		return desc, unpack(self.quest_objective_status[uid][q[desc]])
	end
end
...
function lib:PartyMemberHasQuest(name, uid)
	...
	elseif self.quests[uid] then
		...
		if unit then
			return IsUnitOnQuestByQuestID(uid, unit)
		end
	end
end
